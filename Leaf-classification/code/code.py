# -*- coding: utf-8 -*-
"""
Created on Wed Feb 15 23:47:08 2017
...
class NumpyArrayIterator2(NumpyArrayIterator):
    def next(self):
        ...
        with self.lock:
            # We changed index_array to self.index_array so that we can 
            # match index of the image data and numerical features
...
def combined_model():  
    ...
    x = Convolution2D(8, 5, 5, dim_ordering='th', input_shape=(1, 96, 96), border_mode='same')(image)
    x = (Activation('relu'))(x)
    x = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(x)

    # Now through the second convolutional layer
    x = (Convolution2D(32, 5, 5, dim_ordering='th', border_mode='same'))(x)
    x = (Activation('relu'))(x)
    x = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(x)
...
# autosave best Model
best_model_file = "../output/leafnet.h5"
